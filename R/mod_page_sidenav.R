    ...
    if (id %in% c("my_studies", "my_subsets", "messages", "vocabularies", "scripts", "patient_level_data", "aggregated_data")){
      ...
      if (id == "patient_level_data"){
        ...
        observeEvent(input$person_trigger, {
          ...
          if (debug) print(paste0(Sys.time(), " - mod_page_sidenav - observer input$person_trigger"))
        ...
        observeEvent(input$person, {
          ...
          if (debug) print(paste0(Sys.time(), " - mod_page_sidenav - observer input$person"))
          ...
          if (length(input$person$key) == 0){
            ...
            person <- d$person %>% dplyr::mutate(name_display = paste0(person_id, " - ", gender_concept_name)) %>% dplyr::filter(name_display == person_text) %>% dplyr::collect()
          }
          if (length(input$person$key) > 0){
          ...
          if (d$visit_detail %>% dplyr::count() %>% dplyr::pull() > 0) if (d$visit_detail %>% dplyr::filter(person_id == person_key) %>% dplyr::count() %>% dplyr::pull() == 0) no_stay_available <- TRUE
          ...
          if (!no_stay_available){
            ...
            visit_detail <- d$visit_detail %>% dplyr::filter(person_id == !!person_id) %>% dplyr::collect() %>% dplyr::arrange(visit_detail_start_datetime)
