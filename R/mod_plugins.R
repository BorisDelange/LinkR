...
mod_plugins_server <- function(id = character(), r = shiny::reactiveValues(), d = shiny::reactiveValues(), m = shiny::reactiveValues(),
  ...
  moduleServer(id, function(input, output, session){
    ...
    # 
    #   req(shiny.router::get_page() == "plugins")
    ...
    observeEvent(input$selected_plugin, {
      ...
      # Hide all editors
      sapply(paste0("edit_code_editor_div_", r$edit_plugin_code_editors), shinyjs::hide)
    ...
    observeEvent(r$reload_plugin_code_files, {
      ...
      r$plugin_folder <- paste0(r$app_folder, "/plugins/", r$selected_plugin_type, "/", r$selected_plugin_unique_id)
      ...
      if (nrow(old_plugin_files) > 0){
        for (filename in c("server.R", "ui.R", "translations.csv")){
          file_path <- paste0(r$plugin_folder, "/", filename)
      ...
      if (length(files_list) > 0){
        for (i in 1:length(files_list)){
          ...
          if (file_name %not_in% file_names){
            ...
            options_id <- plugin_files_db %>% dplyr::filter(link_id == r$selected_plugin, name == file_name) %>% dplyr::pull(id)
        ...
        for (i in 1:nrow(plugin_edit_code_files_list)){
          ...
          if (file$filename %not_in% plugin_files_db$name){
              ...
              new_options <- tibble::tibble(
                id = options_new_row_id, category = "plugin_code", link_id = {r$selected_plugin}, name = {file$filename},
                value = NA_character_, value_num = NA_real_, creator_id = r$user_id, datetime = now(), deleted = FALSE
              )
              ...
              new_code <- tibble::tibble(
                id = get_last_row(r$db, "code") + 1, category = "plugin", link_id = {options_new_row_id}, code = file_code,
      ...
      # Initiate plugin editors
      r$edit_plugin_code_editors <- 
        r$plugin_edit_code_files_list %>% 
        dplyr::filter(plugin_id == r$selected_plugin, filename %in% c("server.R", "ui.R", "translations.csv")) %>% 
        dplyr::pull(id)
      ...
      for (filename in c("server.R", "ui.R", "translations.csv")){
        ...
        # if (file_id %not_in% r$edit_plugin_code_editors){
        ui_div <- div(
        ...
        r$edit_plugin_code_editors <- c(r$edit_plugin_code_editors, file_id)
        # }
    ...
    observeEvent(input$edit_code_selected_file, {
      ...
      if (r$edit_plugin_code_open_new_editor){
        ...
        insertUI(selector = paste0("#", ns("edit_code_editors_div")), where = "beforeEnd", ui = div(
          id = ns(paste0("edit_code_editor_div_", file_id)),
          shinyAce::aceEditor(
            ns(paste0("edit_code_editor_", file_id)), value = "", mode = "r",
    ...
    observeEvent(r$edit_plugin_code_reload_files_tab, {
      ...
      if (nrow(r$edit_plugin_code_tabs) > 0){
        for (i in 1:nrow(r$edit_plugin_code_tabs)){
          plugin <- r$edit_plugin_code_tabs[i, ]
          
          tab_class <- "tab"
          if (r$edit_plugin_code_open_new_tab != "none"){
            if (r$edit_plugin_code_open_new_tab == "ui" & plugin$filename == "ui.R") tab_class <- "tab active"
            ...
            if (length(input$edit_code_selected_file) > 0){
              ...
              if (plugin$id == new_plugin_id) tab_class <- "tab active"
            }
          }
          
          tabs_ui <- tagList(tabs_ui, div(
            id = ns(paste0("edit_code_tab_", plugin$id)),
            class = tab_class,
            onclick = paste0("Shiny.setInputValue('", id, "-edit_code_selected_tab', this.id, {priority: 'event'})"),
            plugin$filename,
            div(
              id = ns(paste0("edit_code_clode_tab_", plugin$id)),
              class = "close-tab",
              onclick = paste0("Shiny.setInputValue('", id, "-edit_code_clode_selected_tab', this.id, {priority: 'event'})"),
    ...
    observeEvent(input$edit_code_selected_tab, {
      ...
      shinyjs::delay(50, shinyjs::show(paste0("edit_code_editor_div_", current_file_id)))
      
      # Hide other editors
      hide_ids <- setdiff(r$edit_plugin_code_tabs$id, current_file_id)
    ...
    observeEvent(input$edit_code_add_file, {
      ...
      # Create file
      writeLines("", paste0(r$plugin_folder, "/", new_filename))
      
      # Add file in database
      options_new_row_id <- get_last_row(r$db, "options") + 1
      new_options <- tibble::tibble(
        id = options_new_row_id, category = "plugin_code", link_id = {r$selected_plugin}, name = {new_filename},
        value = NA_character_, value_num = NA_real_, creator_id = r$user_id, datetime = now(), deleted = FALSE
      )
      DBI::dbAppendTable(r$db, "options", new_options)
      ...
      r$edit_plugin_code_editors <- c(r$edit_plugin_code_editors, options_new_row_id)
    })
    ...
    #   plugin <- r$remote_git_plugins %>% dplyr::filter(unique_id == input$plugin_id)
    #   
    #   # Add columns that don't exist (if a language has been added after the creation of the plugin)
    #   prefixes <- c("description", "name", "category")
    #   new_cols <- outer(prefixes, r$languages$code, paste, sep = "_") %>% as.vector()
    #   for(col in new_cols) if(!col %in% colnames(plugin)) plugin <- plugin %>% dplyr::mutate(!!col := "")
    #   
    #   if (plugin[[paste0("name_", language)]] == "") plugin[[paste0("name_", language)]] <- plugin$name_en
    #   
    #   tryCatch({
    #     
    ...
    #     }
    #     
    #     for (name in c(paste0("description_", r$languages$code))) if (name %in% colnames(plugin)) plugin[[name]] <- plugin[[name]] %>% stringr::str_replace_all("'", "''")
    #     
    #     # Plugin table
    #     
    ...
    #             value = plugin[[paste0(col_prefix, "_", code)]],
    ...
    #     
    #     page_id <- id
    #     render_datatable(output = output, ns = ns, i18n = i18n, data = 
    #       data_plugins_datatable %>% dplyr::mutate(
    #         name = glue::glue("<a href='#!/plugins_patient_lvl' class='datatable_link' ",
    #         "onClick = \"Shiny.setInputValue('{page_id}-selected_plugin', {id}, {{priority: 'event'}});\"",
    #         ">{name}</a>")
    #       ),
    #       datatable_dom = "",
    #       output_name = "plugins_datatable", col_names = get_col_names(table_name = "plugins", i18n = i18n),
    #       editable_cols = editable_cols, sortable_cols = sortable_cols, centered_cols = centered_cols, column_widths = column_widths,
    #       searchable_cols = searchable_cols, filter = TRUE, hidden_cols = hidden_cols, selection = "none")
    #     
    ...
    #   sapply(c("plugin_author", "plugin_version", paste0("plugin_name_", r$languages$code), paste0("plugin_category_", r$languages$code)),
    ...
    #   sapply(paste0("plugin_description_", r$languages$code), function(name) shinyAce::updateAceEditor(session, name, value = ""))
    #   
    ...
    #     if (length(value) == 0) value <- ""
    #     if (length(value) > 0) if (is.na(value)) value <- ""
    #     shinyAce::updateAceEditor(session, paste0("plugin_", field), value = value) 
    #   }
    #   
    ...
    #   for (field in c("users_allowed_read_group", "plugin_version", "plugin_author", "plugin_image",
    #     paste0("plugin_name_", r$languages$code), paste0("plugin_category_", r$languages$code),
    #     paste0("plugin_description_", r$languages$code))) data[[stringr::str_replace(field, "plugin_", "")]] <- input[[field]]
    #   
    #   save_settings_options(output = output, r = r, id = id, category = "plugin", code_id_input = paste0("options_", link_id),
    #     i18n = i18n, data = data, page_options = c("users_allowed_read", "version", "author", "image",
    ...
    #   plugin_folder <- paste0(r$app_folder, "/plugins/", prefix, "/", options %>% dplyr::filter(name == "unique_id") %>% dplyr::pull(value))
    #   
    #   options_description <- isolate(input[[paste0("plugin_description_", input$plugin_language)]] %>% 
    #       stringr::str_replace_all("\r", "\n")) %>%
    ...
    #     req(length(lines) > 0)
    #     
    #     start_row <- input[[paste0("ace_edit_code_", type, "_comment")]]$range$start$row + 1
    #     end_row <- input[[paste0("ace_edit_code_", type, "_comment")]]$range$end$row + 1
    #     
    #     for (i in start_row:end_row) if (startsWith(lines[i], "# ")) lines[i] <- substr(lines[i], 3, nchar(lines[i])) else lines[i] <- paste0("# ", lines[i])
    #     
    #     shinyAce::updateAceEditor(session, paste0("ace_edit_code_", type), value = paste0(lines, collapse = "\n"))
    #     
    #     shinyjs::runjs(sprintf("
    #     var editor = ace.edit('%s-ace_edit_code_%s');
    #     editor.moveCursorTo(%d, %d);
    #     editor.focus();
    #       ", id, type, input[[paste0("ace_edit_code_", type, "_comment")]]$range$end$row, input[[paste0("ace_edit_code_", type, "_comment")]]$range$end$column))
    #   })
    # })
    ...
    #     prefixes <- c("description", "name", "category")
    #     new_cols <- outer(prefixes, r$languages$code, paste, sep = "_") %>% as.vector()
    #     for(col in new_cols) if(!col %in% colnames(plugins)) plugins <- plugins %>% dplyr::mutate(!!col := "")
    #     
    #     plugins <- plugins %>% dplyr::mutate(name = get(paste0("name_", language))) %>% dplyr::relocate(id)
    # 
