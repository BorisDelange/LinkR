...
mod_data_server <- function(id){
  ...
  moduleServer(id, function(input, output, session){
    ...
    observe_event(input$add_tab_button, {
      ...
      if (length(tab_name) == 0 || is.na(tab_name) || tab_name == ""){
        shiny.fluent::updateTextField.shinyInput(session, "tab_name", errorMessage = i18n$t("provide_valid_name"))
        return()
      }
      ...
      if (name_already_used){
        shiny.fluent::updateTextField.shinyInput(session, "tab_name", errorMessage = i18n$t("name_already_used"))
        return()
      }
    ...
    observe_event(input$save_tab_button, {
      ...
      if (is.na(tab_name) | tab_name == ""){
        shiny.fluent::updateTextField.shinyInput(session, "edit_tab_name", errorMessage = i18n$t("provide_valid_name"))
        return()
      }
    ...
    observe_event(input$widget_creation_save, {
      ...
      if (length(widget_name) == 0 || is.na(widget_name) || widget_name == "") shiny.fluent::updateTextField.shinyInput(session, "widget_creation_name", errorMessage = i18n$t("provide_valid_name"))
        ...
        if (length(input$selected_element) == 0){
          output$add_widget_selected_plugin <- renderUI(
            div(
              class = "element_widget plugin_widget",
              div(
                class = "element_widget_icon plugin_widget_icon",
        ...
        if (name_already_used){
          shiny.fluent::updateTextField.shinyInput(session, "widget_creation_name", errorMessage = i18n$t("name_already_used"))
          return()
        }
    ...
    observe_event(input$widget_edition_save, {
      ...
      if (length(input$widget_edition_name) == 0) return()
      widget_name <- input$widget_edition_name
      
      # Check if name is not empty
      if (is.na(widget_name) | widget_name == ""){
        shiny.fluent::updateTextField.shinyInput(session, "widget_edition_name", errorMessage = i18n$t("provide_valid_name"))
        return()
      }
      ...
      name_already_used <- remove_special_chars(widget_name) %in% remove_special_chars(widgets_names)
      if (name_already_used){
        shiny.fluent::updateTextField.shinyInput(session, "widget_edition_name", errorMessage = i18n$t("name_already_used"))
        return()
      }
      ...
      if (length(plugin_id) == 0){
        output$edit_widget_selected_plugin <- renderUI(
          div(
            class = "element_widget plugin_widget",
            div(
              class = "element_widget_icon plugin_widget_icon",
