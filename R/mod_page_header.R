  ...
  tagList(
    div(
      ...
      div(
        ...
        div(
          uiOutput(ns("command_bar_1")),
          uiOutput(ns("command_bar_2")),
...
mod_page_header_server <- function(id = character(), r = shiny::reactiveValues(), d = shiny::reactiveValues(), m = shiny::reactiveValues(), language = "en", i18n = character(), debug = FALSE){
  moduleServer(id, function(input, output, session){
    ...
    if (id %in% c(
      "app_db", "home", "console", "datasets", "data_cleaning", "explore", "git_repos",
    ...
    output$command_bar_1 <- renderUI(command_bar_1)
    output$command_bar_2 <- renderUI(command_bar_2)
    
    ## Settings bar ----
    
    output$settings_div <- renderUI(
      div(
        shiny.fluent::CommandBar(
          items = list(
            shiny.fluent::CommandBarItem(
