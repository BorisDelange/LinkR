...
add_settings_new_data <- function(session, output, r = shiny::reactiveValues(), d = shiny::reactiveValues(), m = shiny::reactiveValues(),
  ...
  sapply(req_unique_values, function(field){
    ...
    if (tolower(data[[field]]) %in% distinct_values) {
      ...
      if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = paste0(field, "_already_used"), type = "severeWarning", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
    }
  ...
  if (!dropdowns_check){
    ...
    if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = "dropdown_empty", type = "severeWarning", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  }
  req(dropdowns_check)
  ...
  if (table %in% c("datasets", "subsets")){
    ...
    new_data$data <- new_data$data %>% dplyr::bind_cols(tibble::tribble(~creator_id, ~datetime, ~deleted, r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE))
  }
  
  if (table == "datasets"){
    # new_data$data <- tibble::tribble(
    #   ~id, ~name, ~data_source_id, ~creator_id, ~creation_datetime, ~update_datetime, ~deleted,
    #   last_row$data + 1, as.character(data$name), as.integer(data$data_source), r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
    new_data$data <- tibble::tribble(
      ~id, ~name, ~data_source_id, ~creator_id, ~creation_datetime, ~update_datetime, ~deleted,
      last_row$data + 1, as.character(data$name), NA_integer_, r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  # Creation of new_data$data variable for vocabulary page
    ...
    #   data$data_source, r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
    new_data$data <- tibble::tribble(~id, ~vocabulary_id, ~vocabulary_name, ~vocabulary_reference, ~vocabulary_version,
      ~vocabulary_concept_id, ~display_order, ~data_source_id, ~creator_id, ~creation_datetime, ~update_datetime, ~deleted,
      last_row$data + 1, as.character(data$vocabulary_id), as.character(data$vocabulary_name), "", "", NA_integer_,
      NA_integer_, "", r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  # Creation of new_data$data variable for studies page
    ...
    new_data$data <- tibble::tribble(
      ...
      r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  # Creation of new_data$data variable for plugins page
    ...
    new_data$data <- tibble::tribble(~id, ~name, ~tab_type_id, ~creation_datetime, ~update_datetime, ~deleted,
      last_row$data + 1, as.character(data$name), as.integer(data$tab_type), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  # Creation of new_data$data variable for scripts page
    ...
    new_data$data <- tibble::tribble(~id, ~name, ~creation_datetime, ~update_datetime, ~deleted,
      last_row$data + 1, as.character(data$name), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  # Password is hashed
    ...
    new_data$data <- tibble::tribble(~id, ~username, ~firstname, ~lastname, ~password, ~user_access_id, ~user_status_id, ~datetime, ~deleted,
      ...
      rlang::hash(data$password), as.integer(data$user_access), as.integer(data$user_status), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  
  else if (table %in% c("users_accesses", "users_statuses")){
    new_data$data <- tibble::tribble(~id, ~name, ~description, ~datetime, ~deleted,
      last_row$data + 1, as.character(data$name), as.character(data$description), format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
    ...
    new_data$data <- tibble::tribble(~id, ~category, ~name,  ~description, ~tab_group_id, ~parent_tab_id,  ~display_order, ~creator_id, ~datetime, ~deleted,
      ...
      as.integer(data$display_order), r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  #     last_row$data + 1, category, as.character(data$name), as.character(data$description), r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  # }
    ...
    new_data$data <- tibble::tribble(~id, ~unique_id, ~name, ~api_key, ~repo_url_address, ~raw_files_url_address, ~creator_id, ~datetime, ~deleted,
      ...
      as.character(data$repo_url_address), as.character(data$raw_files_url_address), r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  if (table %in% c("datasets", "vocabulary")){
    new_data$code <- tibble::tribble(~id, ~category, ~link_id, ~code, ~creator_id, ~datetime, ~deleted,
      last_row$code + 1, get_singular(word = table), last_row$data + 1, "", r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  if (grepl("plugins", id)){
    ...
    dplyr::mutate(creator_id = r$user_id, datetime = format(Sys.time(), "%Y-%m-%d %H:%M:%S"), deleted = FALSE)
    
    # Add code rows
    new_data$code <- tibble::tibble(
      id = last_row$code + 1:3,
      category = c("plugin_ui", "plugin_server", "plugin_translations"),
      link_id = last_row$data + 1, code = "", creator_id = r$user_id, datetime = format(Sys.time(), "%Y-%m-%d %H:%M:%S"), deleted = FALSE
    )
  }
  ...
  if (table == "scripts"){
    ...
    # Add code row
    new_data$code <- tibble::tribble(~id, ~category, ~link_id, ~code, ~creator_id, ~datetime, ~deleted,
      last_row$code + 1, "script", last_row$data + 1, "", r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  if (table == "studies"){
    ...
    new_data$subsets <- tibble::tribble(~id, ~name, ~description, ~study_id, ~creator_id,  ~datetime, ~deleted,
      last_row$subsets + 1, i18n$t("subset_all_patients"), "", last_row$data + 1, r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
    ...
    new_data$code <- tibble::tribble(~id, ~category, ~link_id, ~code, ~creator_id, ~datetime, ~deleted,
      last_row$code + 1, "subset", last_row$subsets + 1, code, r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
    ...
    new_data$tabs_groups <- tibble::tribble(~id, ~category, ~name, ~description, ~creator_id, ~datetime, ~deleted,
      get_last_row(r$db, "tabs_groups") + 1, "patient_lvl", data$name, "", r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE,
      get_last_row(r$db, "tabs_groups") + 2, "aggregated", data$name, "", r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
    
    # new_data$aggregated_tabs_groups <- tibble::tribble(~id, ~name, ~description, ~creator_id, ~datetime, ~deleted,
    #   get_last_row(r$db, "aggregated_tabs_groups") + 1, data$name, "", r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
    ...
    r$study_page <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
  }
  
  # For subsets, need to add one row in code
  if (table == "subsets"){
    new_data$code <- tibble::tribble(~id, ~category, ~link_id, ~code, ~creator_id, ~datetime, ~deleted,
      last_row$code + 1, "subset", last_row$subsets + 1, "", r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  }
  ...
  #     last_row$options + 1, get_singular(word = table), last_row$data + 1, "users_allowed_read_group", "everybody", 1, r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE,
  #     last_row$options + 2, get_singular(word = table), last_row$data + 1, "user_allowed_read", "", r$user_id, r$user_id, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), FALSE)
  # }
  ...
  if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = paste0(get_singular(table), "_added"), type = "success", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
...
delete_element <- function(r = shiny::reactiveValues(), m = shiny::reactiveValues(), session, input, output, ns = character(), i18n = character(),
  ...
  observeEvent(input[[paste0(delete_prefix, "_delete_confirmed")]], {
    ...
    if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = delete_message, type = "warning", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
    
    # Activate reload variable
    if (length(reload_variable) > 0) r[[reload_variable]] <- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
...
monitor_perf <- function(r = shiny::reactiveValues(), action = "stop", task = character()){
  ...
  if (action == "start") datetime_start <<- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
  
  if (action == "stop"){
    datetime_stop <<- format(Sys.time(), "%Y-%m-%d %H:%M:%S")
    ...
    datetime_start <<- format(Sys.time(), "%Y-%m-%d %H:%M:%S") 
  }
}
    ...
    if ("users_allowed_read" %in% page_options){
      ...
      if (length(data$users_allowed_read) != 0){
        ...
        new_data <- tibble::tibble(id = (last_row + (1:length(data$users_allowed_read))), category = category, link_id = link_id,
          ...
          datetime = format(Sys.time(), "%Y-%m-%d %H:%M:%S"), deleted = FALSE)
    ...
    if (field %in% page_options){
      ...
      if (DBI::dbGetQuery(r$db, sql) %>% nrow() == 0){
        ...
        new_data <- tibble::tibble(
          ...
          name = field, value = new_value, value_num = NA_real_, creator_id = r$user_id, datetime = format(Sys.time(), "%Y-%m-%d %H:%M:%S"), deleted = FALSE)
        print(new_data)
        DBI::dbAppendTable(r$db, "options", new_data)
        r$options <- r$options %>% dplyr::bind_rows(new_data)
      }
...
save_settings_datatable_updates <- function(output, r = shiny::reactiveValues(), m = shiny::reactiveValues(), ns = character(), 
  ...
  if (!duplicates_allowed){
    ...
    if (duplicates_display_order > 0){
      ...
      if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = "modif_display_order_duplicates", type = "severeWarning", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
    } 
    if (tab_is_its_own_parent > 0){
      if (!r_message_bar) show_message_bar(output, "tab_cannot_be_its_own_parent", "severeWarning", i18n, ns = ns)
      if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = "tab_cannot_be_its_own_parent", type = "severeWarning", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
    }
    if (loop_over_tabs > 0){
      if (!r_message_bar) show_message_bar(output, "tab_loop_between_tabs", "severeWarning", i18n, ns = ns)
      if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = "tab_loop_between_tabs", type = "severeWarning", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
    }
    if (duplicates_name > 0){
      if (!r_message_bar) show_message_bar(output, "modif_names_duplicates", "severeWarning", i18n, ns = ns)
      if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = "modif_names_duplicates", type = "severeWarning", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
    }
  ...
  if (names_empty > 0){
    ...
    if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = "names_empty", type = "severeWarning", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  }
  ...
  if (length(ids_to_del) == 0 & table != "concept_user"){
    ...
    if (r_message_bar) r[[paste0(table, "_show_message_bar")]] <- tibble::tibble(message = "modif_saved", type = "success", trigger = format(Sys.time(), "%Y-%m-%d %H:%M:%S"))
  }
