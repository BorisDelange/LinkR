help_scripts <- function(output, r = shiny::reactiveValues(), id = character(), language = "en", i18n = character(), ns = character()){
  ...
  output$help_modal <- shiny.fluent::renderReact({
    ...
    shiny.fluent::Modal(
      isOpen = r$help_scripts_open_modal, dragOptions = TRUE, isModeless = FALSE, topOffsetFixed = TRUE,
  ...
  observeEvent(r$help_scripts_page_2, {
    ...
    if (language == "fr"){
      r$help_scripts_modal_text <- div(
        ...
        p("Vous pouvez accéder à une liste de scripts disponibles sur un git distant (fichiers stockés sur un site type framagit.org, github.com, gitlab.com...)."),
    ...
    if (language == "en"){
      r$help_scripts_modal_text <- div(
        ...
        p("You can access a list of scripts available on a remote git (files stored on sites like framagit.org, github.com, gitlab.com, etc.)."),
